...
@Slf4j
public class RecordDebitResultCommandHandler implements CommandHandler<RecordDebitResultCommand, Void> {
    ...
    @Override
    @Transactional
    public Void handle(RecordDebitResultCommand command) {
        log.info("Handling debit leg result for transaction {}", command.getTransactionId());
