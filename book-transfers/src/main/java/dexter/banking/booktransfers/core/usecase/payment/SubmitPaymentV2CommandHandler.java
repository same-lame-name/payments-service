...
public class SubmitPaymentV2CommandHandler implements CommandHandler<PaymentCommand, PaymentResponse> {
    ...
    private TransactionOrchestratorPort findOrchestrator(ModeOfTransfer mode) {
        String beanName = switch (mode) {
            ...
            default -> throw new IllegalArgumentException("V2 transactions must use an orchestrated mode (SYNC_STATE_MACHINE or ASYNC_STATE_MACHINE), but got: '" + mode + "'");
        };
